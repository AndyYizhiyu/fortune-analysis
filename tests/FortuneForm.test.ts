...
function mockFetchWithHistory(prompt = "结构化命理提示词") {
  return vi.fn(async (url: string, options?: RequestInit) => {
          ...
          items: [
            {
              ...
              preview: "历史提示词内容历史提示词内容历史提示词内容历史提示词内容历史提示词",
            },
          ],
        }),
      };
    ...
    if (url === "/api/history/history_000") {
      return {
        ok: true,
        json: async () => ({
          id: "history_000",
          createdAt: "2026-05-08 16:00",
          optimizedPrompt: "历史提示词完整内容",
        }),
      };
    }
...
describe("FortuneForm", () => {
  ...
  it("支持暗黑模式切换", async () => {
    ...
    expect(wrapper.get('[data-test="theme-toggle"]').text()).toContain("亮色");
  });
  ...
  it("展示最近生成的历史记录前50字，不展示用户输入，并支持查看详情", async () => {
    ...
    expect(wrapper.text()).toContain("历史提示词内容历史提示词内容历史提示词内容历史提示词内容历史提示词");
